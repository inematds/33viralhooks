/* Trilha 1 — Fundamentos (Emerald): index + 3 módulos.
   Define CADA termo inline (erro #31) e ilustra cada conceito (erro #29). */
...
${data('Como o hook mexe em cada métrica',['<strong>Retenção:</strong> um hook forte segura os 3s e puxa a curva de retenção pra cima — o sinal nº 1 de qualidade.','<strong>Watch-time:</strong> mais gente segurada × mais segundos = mais watch-time total, o combustível do alcance.','<strong>CTR / scroll-stop rate:</strong> hooks visuais e contrarian aumentam a fração que para em vez de deslizar.'])}
...
${data('Exemplo desmontado — "A IA substituiu um cargo de US$200 mil… mas o chocante não é isso."',['<strong>Gancho:</strong> "IA substituiu um cargo de US$200 mil" (ameaça + número).','<strong>Tensão:</strong> "mas o chocante não é isso" (abre uma lacuna: então o que é?).','<strong>Promessa:</strong> existe uma revelação maior esperando — fique pra ver.'])}
${tip('Faltou uma engrenagem? O hook capenga.','Só gancho = choca e esvazia. Só promessa = soa vago. A tensão é a cola que segura entre o choque e a entrega.')}`}),
...
${data('O que a plataforma observa nos primeiros segundos',['Quantos deslizaram antes de 3s (quanto menos, melhor).','A inclinação da curva de retenção logo no começo.','Re-watches e idas pro perfil — sinais de que o início valeu.'])}
...
${L.moduleSummary({acc,points:[
  ...
  return {file:`curso/trilha1/modulo-${mod}.html`,html:L.shell({title:'Anatomia de um hook',depth:2,activeTrack:1,accents:['emerald'],body})};
}
...
${data('O que faz uma boa frase falada',['Cabe em ~2 segundos de fala (curta).','Abre uma lacuna ("…mas não é isso que choca").','Usa específico (número, nome, valor) em vez de adjetivo vago.'])}
...
${data('Regras do Text Hook',['Curto: 2-3 palavras. Se não cabe num olhar, é longo demais.','Alto contraste, fonte grande, no terço superior ou central.','Reforça a tensão da frase falada, não repete ela inteira.'])}
...
${L.moduleSummary({acc,points:[
  ...
  return {file:`curso/trilha1/modulo-${mod}.html`,html:L.shell({title:'As 3 camadas do hook',depth:2,activeTrack:1,accents:['emerald'],body})};
}
...
${data('O que gera scroll stop',['Imagem com tensão (algo prestes a dar errado, um contraste forte).','Afirmação que parece impossível ou perigosa.','Número específico demais pra ser inventado.'])}
...
${L.moduleSummary({acc,points:[
  ...
  return {file:`curso/trilha1/modulo-${mod}.html`,html:L.shell({title:'A fórmula e os 5 nichos',depth:2,activeTrack:1,accents:['emerald'],body})};
}

function svgPanel(svg){ return L.svgPanel('emerald', svg); }
